...
# ╔═╡ ba557ccc-5ab8-11ec-2f59-936591af7162
using DataFrames

# ╔═╡ db932468-39cf-4ca2-a4e1-73499ef73784
using StatsPlots

# ╔═╡ 9cb78f44-1e4d-4c51-8bb3-c80e05587bcf
using Statistics

# ╔═╡ 8504dcf1-897d-4ca9-ad2f-6203c6c49453
begin
	regex = r"\[(?<when>.*?)\].*(?<what>Young|Full|Mark-Compact).*\((?:unknown|Allocation).*?\)\s+(?<from>\d+.)->(?<to>\d+.)\((?<total>\d+.)\)\s+(?<time>.*)"
	timeunit = Dict(["ms" => 1, "s" => 1000])
	function parsetime(time)
	  unit = match(r"(m?s)", time)[1]
	  time = replace(time, unit => "")
	  time = replace(time, "," => ".")
	  parse(Float64, time) * timeunit[unit]
	end
	
	memoryunit = Dict(["M" => 1])
	function parsememory(memory)
	  unit = match(r"(M)", memory)[1]
	  memory = replace(memory, unit => "")
	  parse(Float64, memory) * memoryunit[unit]
	end
	function matchgclog(line, gc)
	  m = match(regex, line)
	  isnothing(m) && return nothing
	  (
	    when = parsetime(m[:when]), 
	    what = m[:what],
	    from = parsememory(m[:from]), 
	    to = parsememory(m[:to]), 
	    total = parsememory(m[:total]), 
	    time = parsetime(m[:time]),
		gc=gc
	  )
	end
	parselogs(gc) = DataFrame(filter(!isnothing, matchgclog.(readlines("$(gc)_logs"), gc)))
end
